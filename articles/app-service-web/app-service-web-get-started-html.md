...
Ten samouczek ułatwia wdrażanie prostych aplikacji sieci web HTML + CSS do [Azure aplikacji usługi](../app-service/app-service-value-prop-what-is.md).
...
- Wdrażanie kodu HTML i arkuszy CSS do niego.
- Zobacz stron systemem live produkcji.
- Aktualizowanie zawartości tak samo jak [wypychania zatwierdza cyfra](https://git-scm.com/docs/git-push).
...
## <a name="deploy-a-simple-html-site"></a>Wdrażanie prostej witrynie HTML
...
4. Wdrażanie kodu przykładowe Azure aplikacji tak samo push kodu z cyfra. Gdy zostanie wyświetlony monit, użyj hasła, które skonfigurowano wcześniej.
    ...
    Jeśli użyto RAM język pojawi się inny wynik. Dzieje się tak dlatego `git push` nie tylko umieszcza kod w Azure, ale również wyzwalane zadania rozmieszczania w aparatu wdrażania. Jeśli masz dowolnego package.json (Node.js) lub requirements.txt (Python) plików w główną projektu (repozytorium) lub jeśli plik packages.config w projekcie ASP.NET, skrypt wdrożenia przywraca wymagane pakiety dla Ciebie. Można także [włączyć rozszerzenie Kompozytor](web-sites-php-mysql-deploy-use-git.md#composer) automatyczne przetwarzanie plików composer.json w aplikacji PHP.
...
Znajdź preferowanej kroki projektowania i wdrażania dla programu framework język:
