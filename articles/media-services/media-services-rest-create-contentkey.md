<properties 
    pageTitle="Tworzenie ContentKeys z pozostałych | Microsoft Azure" 
...
#<a name="create-contentkeys-with-rest"></a>Tworzenie ContentKeys z pozostałych
...
Usługi multimediów umożliwia tworzenie nowych i przeprowadzania zaszyfrowanych składników majątku. **ContentKey** zapewnia bezpieczny dostęp do Twojej s **zawartości**. 

Podczas tworzenia nowych elementów zawartości (na przykład przed [przekazywać pliki](media-services-rest-upload-files.md)), można określić następujące opcje szyfrowania: **StorageEncrypted**, **CommonEncryptionProtected**lub **EnvelopeEncryptionProtected**. 

Podczas wyświetlania elementów dla klientów, można [skonfigurować dla trwałych dynamicznie szyfrowania](media-services-rest-configure-asset-delivery-policy.md) z jednym z następujących dwóch skonfigurowała: **DynamicEnvelopeEncryption** lub **DynamicCommonEncryption**.
...
1. Wygenerować losowo 16-bajtowe AES (szyfrowania wspólnej i koperty) 32-bajtowa AES klawisz lub (w przypadku szyfrowania miejsca do magazynowania). 
...
3.  Szyfrowanie klucza zawartości z kluczem certyfikat X.509. 
    ...
    Multimedia usług .NET SDK używa RSA z OAEP podczas szyfrowania.  Przykład [funkcji EncryptSymmetricKeyData](https://github.com/Azure/azure-sdk-for-media-services/blob/dev/src/net/Client/Common/Common.FileEncryption/EncryptionUtils.cs)jest widoczny.
4.  Tworzenie sumy kontrolnej wartości (na podstawie PlayReady AES klucza sumy kontrolnej algorytmu) obliczane za pomocą identyfikatora klucza i klucza zawartości. Aby uzyskać więcej informacji, zobacz sekcję "Algorytm sumy kontrolnej klucza AES PlayReady" dokumentu PlayReady nagłówka obiektu umieszczane [w tym miejscu](http://www.microsoft.com/playready/documents/).

    W poniższym przykładzie .NET oblicza sumy kontrolnej, używając GUID część identyfikatora klucza i wyczyść pole wyboru zawartości klucza.
...
6. Kojarzenie jednostki **ContentKey** z jednostka **zawartości** za pomocą operacji $links.

Uwaga Przykłady wygenerowania klucza AES, szyfrowania klucza i obliczanie sumy kontrolnej zostały pominięte z tego tematu. Dostępne są tylko przykłady pokazujące, jak korzystać z usługi multimediów.
...
Jedną z wartości, że należy ustawić podczas tworzenia zawartości klucza jest typem. Wybierz jedną z następujących wartości.
