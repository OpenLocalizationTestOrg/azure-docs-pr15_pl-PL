<properties 
    pageTitle="Wykonywania pracy awaryjnej streaming scenariusz | Microsoft Azure" 
    description="W tym temacie przedstawiono sposób wykonania pracy awaryjnej streaming scenariusza." 
...
#<a name="implementing-failover-streaming-scenario"></a>Wykonywania pracy awaryjnej streaming scenariusz

W tym instruktażu przedstawiono sposób na kopiowanie zawartości (BLOB) z jednego środka trwałego do drugiej w celu obsługi nadmiarowości przesyłania strumieniowego na żądanie. W tym scenariuszu jest przydatna dla klientów, do których chcesz skonfigurować ich CDN do pracy awaryjnej między dwoma centrach danych w przypadku awarii w jednej z naszych centrach danych.
W tym instruktażu używa SDK usługi multimediów Azure firmy Microsoft, interfejsu API usługi REST usługi Microsoft Azure multimediów i SDK miejsca do magazynowania Azure wykazać następujące zadania.

1. Konfigurowanie konta usługi multimediów w "Centrum danych A".
1. Przekaż plik kodery do zawartości źródła.
1. Kodowanie elementu do wielu bitów szybkość MP4, pliki. 
1. Tworzenie tylko do odczytu locator skojarzeń zabezpieczeń dla trwały źródła ma mieć dostęp do odczytu w kontenerze na koncie miejsca do magazynowania, skojarzonego z elementu źródło.
1. Nazwa kontenera trwałego źródła przejść z locator skojarzenia zabezpieczeń tylko do odczytu, utworzony w poprzednim kroku. Potrzebujemy tych informacji do kopiowania obiektów blob między kontami miejsca do magazynowania (opisano w dalszej części tematu).
1. Tworzenie locator origin trwałego, który został utworzony przez kodowania zadania. 

Następnie, aby obsługiwać tym przełączeniu:

1. Konfigurowanie konta usługi multimediów w "B centrum danych".
1. Utwórz pusty trwały docelową w celu konta usługi multimediów.
1. Tworzenie zapisu locator skojarzeń zabezpieczeń dla pustych elementów zawartości docelowej ma uprawnienia do zapisu w kontenerze w celu konta miejsca do magazynowania, które jest skojarzone z elementu docelowego.
1. Kopiowanie obiektów blob (zawartości plików) między kontem miejsca do magazynowania źródła w "Centrum danych A" i konta docelowego miejsca do magazynowania w "B centrum danych" za pomocą Azure miejsca do magazynowania SDK (tych kont przestrzeni dyskowej są skojarzone z aktywów odsetek).
1. Kojarzenie BLOB (zawartości plików), które zostały skopiowane do kontenera obiektów blob docelowego z elementu docelowego. 
1. Utwórz locator origin dla trwałego w "B centrum danych" i określ locator identyfikatora, który został wygenerowany dla elementów zawartości w "Centrum danych A". 
1. Umożliwia przesyłanie strumieniowe adresy URL miejsce, w którym względne ścieżki adresy URL są takie same (tylko do podstawy adresy URL są różne). 
 
Następnie obsługiwać wszystkie dostawie, można utworzyć CDN u góry tych Locator pochodzenia. 

Następujące kwestie:

- Bieżącą wersję pakietu SDK usługi multimediów nie obsługuje tworzenia locator przy użyciu identyfikatora określonej locator. Aby osiągnąć to zadanie, użyjemy interfejsu API usługi REST usługi multimediów.
- Bieżącą wersję pakietu SDK usługi multimediów nie obsługuje programowy generowania informacje IAssetFile skojarzyć środka trwałego z plikami zasobów. Aby osiągnąć to zadanie, użyjemy interfejsu API usługi REST CreateFileInfos multimediów usług. 
- Składniki majątku szyfrowane przestrzeni dyskowej (AssetCreationOptions.StorageEncrypted) nie są obsługiwane dla replikacji (ponieważ klucza szyfrowania będą różne w obu kont usługi multimediów). 
- Jeśli chcesz korzystać z opakowania dynamiczne, należy pobrać co najmniej jeden na żądanie Streaming zastrzeżone jednostki. Aby uzyskać więcej informacji zobacz [Dynamicznie opakowań aktywów](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Warto rozważyć użycie usługi multimediów [Narzędzie Replikator](http://replicator.codeplex.com/) jako alternatywa do wykonania pracy awaryjnej scenariusz przesyłanie strumieniowe ręcznie. To narzędzie umożliwia powielić elementy zawartości przez dwa konta usługi multimediów.

##<a name="prerequisites"></a>Wymagania wstępne
 
- Dwa konta usługi multimediów Azure subskrypcji nowej lub istniejącej. Dowiedz się, [jak utworzyć konto usługi multimediów](media-services-portal-create-account.md).
- Systemy operacyjne: Windows 7, Windows 2008 R2 lub Windows 8.
- .NET framework 4,5 lub .NET Framework 4.
- Visual Studio 2010 z dodatkiem SP1 lub nowszym (Professional, Premium, Ultimate lub Express).
 
##<a name="set-up-your-project"></a>Konfigurowanie projektu

W tej sekcji możesz utworzyć i skonfigurować projekt aplikacji konsoli C#.

1. Visual Studio umożliwia utworzenie nowego rozwiązania zawierający projekt aplikacji konsoli C#. Wprowadź HandleRedundancyForOnDemandStreaming dla nazwy, a następnie kliknij przycisk OK.
1. Tworzenie folderu SupportFiles na tym samym poziomie co HandleRedundancyForOnDemandStreaming.csproj pliku projektu. W folderze SupportFiles tworzyć foldery OutputFiles i MP4Files. Skopiuj plik MP4 do folderu MP4Files (w tym przykładzie, który zostanie użyty plik BigBuckBunny.mp4). 
1. **Nuget** umożliwia dodawanie odwołania do powiązanych DLL usługi multimediów. W Menu głównym Visual Studio, wybierz pozycję Narzędzia -> Menedżer pakietów biblioteki -> konsoli Menedżera pakietów. W oknie konsoli wpisz pakiet instalacyjny windowsazure.mediaservices i naciśnij klawisz enter.
1. Dodawanie innych odwołaniami, które są wymagane dla tego projektu: System.Configuration, System.Runtime.Serialization i System.Web.
1. Zastępowanie przy użyciu instrukcji, które zostały dodane do pliku Programs.cs domyślnie z nich następujące czynności:
...
1. Dodawanie sekcji appSettings do pliku .config i aktualizowanie wartości według klucza usługi multimediów i miejsca do magazynowania i nazwę wartości. 
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Dodaj kod, który obsługuje nadmiarowości przesyłania strumieniowego na żądanie.



1. Dodaj następujące pola poziomie klasy do klasy programu.
...
1. Zastąpić domyślnej definicji metody główne następujące czynności:
...
1. Poniżej przedstawiono definicji metod, które są nazywane z głównego.
...
Za pomocą Menedżera ruch rozsyłanie żądań między centrami danych dwóch, a więc powrotu dowolnego dostawie można teraz.
