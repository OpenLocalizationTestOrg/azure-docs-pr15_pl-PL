...
Koncentratory zdarzeń udostępnia pobrania na dużą skalę, utrzymywanie i przetwarzanie zdarzeń dane ze źródeł danych wysokiej przepustowości i/lub miliony urządzenia. Jeśli z kolejki Bus usługi i tematów, koncentratory zdarzenia umożliwia trwałych polecenia i kontrolowanie wdrożeń programu scenariusze [Internet czynności (IoT)](https://azure.microsoft.com/services/iot-hub/) .
...
Jeśli przepustowość ingress całkowita lub prędkości zdarzenia sumy wejściowej między wszystkimi koncentratorami zdarzenia w obszarze nazw przekracza dodatki jednostki Agreguj przepustowość, nadawcy będą ograniczenie i błędów, wskazująca, że przekroczono przydział ingress.

Jeśli przepustowość całkowita wyjściowego lub stopa wyjściowego całkowita wydarzenia między wszystkimi koncentratorami zdarzenia w obszarze nazw przekracza dodatki jednostki Agreguj przepustowość, odbiorców są ograniczenie i błędów, wskazująca, że przekroczono przydział wyjściowego. Ingress i wyjściowego przydziały są wymuszane oddzielnie, tak aby nadawcy może powodować zużycie zdarzenia zwolnić ani słuchawka zapobiec zdarzeń wysyłana do koncentratora zdarzenia.
...
Istnieje domyślny przydział 20 jednostek przepustowość na przestrzeń nazw. Możesz przejąć normę większych jednostek przepustowość, zachowując bilet pomocy technicznej. Poza limit jednostek 20 przepustowość pakiety są dostępne w jednostkach przepustowość 20 i 100. Należy zauważyć, że korzystanie z więcej niż 20 jednostek przepustowość usuwa możliwość zmiany liczby jednostek przepustowość bez zgłoszenia bilet pomocy technicznej.

## <a name="is-there-a-charge-for-retaining-event-hubs-events-for-more-than-24-hours"></a>Czy istnieje opłatę za zachowywanie zdarzeń koncentratory zdarzeń dla więcej niż 24 godziny?

Zdarzenia koncentratory standardowy warstwa zezwala przechowywania wiadomości okresy dłuższe niż 24 godziny, maksymalnie przez 30 dni. Jeśli rozmiar całkowitej liczby przechowywane zdarzenia przekracza odliczenie miejsca do magazynowania dla liczby jednostek wybranej przepustowości (84 GB na jednostkę przepustowość), rozmiar, którego rozmiar przekracza dodatek jest rozliczana według stawki opublikowanych obiektów Blob platformy Azure miejsca do magazynowania. Dodatek miejsca do magazynowania w każdej jednostki przepustowość obejmuje wszystkie koszty miejsca do magazynowania dla okresów przechowywania 24 godzin (ustawienie domyślne), nawet jeśli jednostka przepustowość jest używany w górę do odliczenie ingress maksymalna.

## <a name="what-is-the-maximum-retention-period"></a>Co to jest okres przechowywania maksymalną?

Warstwa koncentratory standardowy zdarzenie obsługuje obecnie okres przechowywania maksymalną 7 dni. Uwaga koncentratory zdarzenia nie są przeznaczone jako Magazyn trwały danych. Okresy przechowywania większe niż 24 godziny są przeznaczone do scenariusze, w których jest wygodny do ponownego odtworzenia strumienia zdarzenia, w tym samym systemów; na przykład, aby przeszkolenie lub sprawdzanie nowy model nauki komputera na istniejące dane.
...
Tak, jak wszystkie koncentratory zdarzenia, kolejkach i tematy są w tym samym nazw. Jako takie można zaimplementować dwukierunkowego, brokered łączności z wielu urządzeń z subsecond opóźnienia w sposób efektywny pod względem kosztów i wysoce skalowalna.
...
## <a name="geographic-availability"></a>Dostępność geograficzne
