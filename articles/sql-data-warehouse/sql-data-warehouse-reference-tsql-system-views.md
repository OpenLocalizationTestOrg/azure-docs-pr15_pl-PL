<properties
   pageTitle="Widoki magazynu danych SQL systemu | Microsoft Azure"
   description="Łącza do systemu wyświetla zawartość dla magazynu danych SQL."
...
## <a name="sql-data-warehouse-catalog-views"></a>Widoków wykazu magazynu danych SQL
...
## <a name="sql-data-warehouse-dynamic-management-views-dmvs"></a>Dynamiczne zarządzanie magazynem danych SQL widoków (DMVs)
...
## <a name="sql-server-dmvs-applicable-to-sql-data-warehouse"></a>Program SQL Server DMVs stosowane do magazynu danych SQL

Następujące DMVs mają zastosowanie do magazynu danych SQL, ale muszą zostać wykonane za pomocą połączenia z **wzorcem** bazy danych.
...
## <a name="sql-server-dmvs-available-in-sql-data-warehouse"></a>Program SQL Server DMVs dostępne w magazynie danych SQL

Program SQL Data Warehouse udostępnia wiele widoków dynamicznego zarządzania programu SQL Server (DMVs). Te widoki kwerendy w magazynie danych SQL, zgłosić stan uruchomionych podziału bazy danych SQL.

Ponieważ program SQL Data Warehouse jest oparta na technologii MPP firmy Microsoft, zarówno magazynu danych SQL i analizy platformy systemu równoległe danych magazynu (PDW) za pomocą samego widoków systemu.

Jest to, dlatego każdy z tych DMV ma określone kolumnę o nazwie pdw_node_id. To jest identyfikator węzła obliczeń. Węzeł jest silniejsza koncepcji architektury w PDW obliczeń. W magazynie danych SQL architektura bardziej intensywnie korzysta z dystrybucji.

>[AZURE.NOTE] Aby użyć tych widoku, wstawianie "pdw_nodes_" do nazwy, jak pokazano w poniższej tabeli.


| Nazwa DMV w magazynie danych SQL | Łącze do tematu SQL Server Transact-SQL w witrynie MSDN |
| :----------------------------- | :-------------------------------------------- |
...
## <a name="sql-server-2016-polybase-dmvs-available-in-sql-data-warehouse"></a>Program SQL Server 2016 PolyBase DMVs dostępne w magazynie danych SQL
...
## <a name="sql-server-informationschema-views"></a>Widoki programu SQL Server w widoku INFORMATION_SCHEMA
